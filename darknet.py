...
def load_network(config_file, data_file, weights, batch_size=1):
    ...
    network = load_net_custom(
        ...
        weights.encode("utf-8"), 0, batch_size)
...
def draw_boxes_chinese(detections, image, colors, font_path):
    import cv2
    # using PIL to draw Chinese
    from PIL import Image, ImageDraw, ImageFont
    import numpy as np
    _font = ImageFont.truetype(font_path, 20, encoding="utf-8")

    for label, confidence, bbox in detections:
        left, top, right, bottom = bbox2points(bbox)
        cv2.rectangle(image, (left, top), (right, bottom), 1, 1)
        # cv2.putText(image, "{} [{:.2f}]".format(label, float(confidence)),
        #             (left, top - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
        #             colors[label], 2) colors[label]
        pilimg = Image.fromarray(image)
        draw = ImageDraw.Draw(pilimg)
        draw.text((left, top - 20),
                    "{} [{:.2f}]".format(label, float(confidence)), (180, 50, 50), font=_font)
